...
-- The issue: workspaces policy references proposals, and proposals policy references workspaces
-- Solution: Break the circular dependency

-- ============================================================
-- STEP 1: Simplify proposal_read policy (remove workspace reference)
-- ============================================================
...
-- ============================================================
-- STEP 2: Keep workspaces policy as-is (it's correct)
-- ============================================================
-- The workspaces policy already allows team members via proposal_team_members
-- No changes needed here

-- ============================================================
-- STEP 3: Ensure document_collaborators are synced to proposal_team_members
-- ============================================================
-- This is the key: instead of checking document access in the proposal policy,
-- we ensure all document collaborators are in proposal_team_members

-- Sync existing document collaborators to proposal_team_members
...
-- ============================================================
-- STEP 4: Create trigger to auto-sync (without circular dependency)
-- ============================================================
...
-- ============================================================
-- STEP 5: Create trigger to remove team members (without recursion)
-- ============================================================
...
SELECT 
    ...
    cmd
FROM pg_policies 
WHERE tablename IN ('proposals', 'workspaces')
...
-- Check team member sync status
SELECT 
    'Total document collaborators' as metric,
    COUNT(*) as count
FROM document_collaborators dc
...
SELECT 
    'Synced to proposal_team_members' as metric,
    COUNT(*) as count
...
SELECT 
    'Missing from proposal_team_members' as metric,
    COUNT(*) as count
FROM document_collaborators dc
INNER JOIN workspace_documents wd ON wd.id = dc.document_id
INNER JOIN workspaces w ON w.id = wd.workspace_id
